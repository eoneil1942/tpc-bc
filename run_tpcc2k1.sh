...
function catalog-cv() {
    srccompile
    $VOLTCOMPILER obj tpcc-cv-project.xml $APPNAME.jar
    # stop if compilation fails
    if [ $? != 0 ]; then exit; fi
}
function catalog-v() {
    srccompile
    $VOLTCOMPILER obj tpcc-project.xml $APPNAME.jar
    # stop if compilation fails
    if [ $? != 0 ]; then exit; fi
}
...
function server() {
    ...
    # run the server
    $VOLTDB create catalog $APPNAME.jar deployment mh2_deploymentk1.xml \
